...
void* operative_thread(void* arg) {
    ...
    printf("Operative %d has arrived at typewriting station %d at time %.0f\n", 
           op->id, op->station_id, arrival_time);
    
    // Wait for typewriting station
    sem_wait(&station_semaphores[op->station_id - 1]);
    ...
    printf(RED"Operative %d has completed document recreation at time %.0f\n", 
           op->id, completion_time);
    printf(RESET);
    // Signal station availability
    sem_post(&station_semaphores[op->station_id-1]);
    
    // Update unit completion count
    pthread_mutex_lock(&unit_mutex[op->unit_id]);
    unit_completion_count[op->unit_id]++;

    // Check if this operative is the unit leader
    int unit_leader_id = (op->unit_id + 1) * M; // Highest ID in this unit
    bool is_leader = (op->id == unit_leader_id);
    ...
    // If I'm the leader, I need to wait for my unit to complete (if not already)
    if (is_leader) {
        // Wait until my unit is complete
        while (unit_completion_count[op->unit_id] < M) {
            pthread_mutex_unlock(&unit_mutex[op->unit_id]);
            sleep(1); // Brief wait to avoid busy waiting
            pthread_mutex_lock(&unit_mutex[op->unit_id]);
        }
        
        // Now the unit is complete and I'm the leader - write to logbook
        printf("Unit leader (Operative %d) writing to logbook...\n", op->id);
        
        // Release unit mutex before waiting for write semaphore to avoid deadlock
        pthread_mutex_unlock(&unit_mutex[op->unit_id]);
        ...
        printf(GREEN"Unit %d has completed intelligence distribution at time %.0f\n",
            op->unit_id + 1, get_current_time());
        printf(RESET);
        ...
        // Non-leader operatives just finish
        printf("Operative %d finished but unit leader will handle logbook\n", op->id);
        pthread_mutex_unlock(&unit_mutex[op->unit_id]);
    }
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        operatives[i].station_id = (operatives[i].id % 4)+1; // Station assignment: (ID mod 4)
